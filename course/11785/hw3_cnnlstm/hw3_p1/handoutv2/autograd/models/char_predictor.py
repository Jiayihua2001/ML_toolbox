...
class CharacterPredictor(object):
    ...
    def __init__(self, input_dim, hidden_dim, num_classes, autograd_engine):
        ...
        self.gru =  GRUCell(input_dim, hidden_dim,self.autograd_engine)
        self.projection = Linear(hidden_dim,num_classes,self.autograd_engine)
...
def inference(net, inputs):
    ...
    logits=[]
    h = np.zeros(net.gru.h)
    for t in range(seq_len):
        input_t =inputs[t]
        logit,h = net(input_t,h)
        logits.append(logit)
