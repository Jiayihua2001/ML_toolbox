...
class CTC(object):
    ...
    def get_forward_probs(self, logits, extended_symbols, skip_connect):
        ...
        for t in range(1,T):
            for sym in range(S):
                alpha[t][sym] = logits[t, extended_symbols[sym]]*alpha[t-1,sym]
                if sym >0:
                    alpha[t,sym] +=alpha[t-1,sym-1]*logits[t, extended_symbols[sym]]
    ...
    def get_posterior_probs(self, alpha, beta):
        ...
        for t in range(T):
            sumgamma[t] = np.sum(alpha[t, :] * beta[t, :])
            gamma[t, :] = (alpha[t, :] * beta[t, :]) / sumgamma[t]
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            truncated_logits = logits[:input_lengths[batch_itr], batch_itr, :]
            ...
            self.gammas = gamma  # Store gamma for backward pass

            total_loss[batch_itr] = -np.sum(np.log(gamma))
