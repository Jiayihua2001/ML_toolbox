...
def ctc_loss_backward(grad, logits, input_lengths, gammas, extended_symbols):
    ...
    gammas [list of np.array]:
        intermediate values used in the forward pass of CTC

    extended_symbols [list of np.array]:
        target sequences extended with blank symbols
    ...
    for batch_itr in range(B):
        ...
        for r in range(gamma.shape[1]):
            symbol_index = ext_symbols[r]
            dY[:input_lengths[batch_itr], batch_itr, symbol_index] -= (
                grad * gamma[:, r] / logits_t[:, symbol_index]
            )
