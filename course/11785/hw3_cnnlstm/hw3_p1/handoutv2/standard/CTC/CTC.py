...
class CTC(object):
    ...
    def get_forward_probs(self, logits, extended_symbols, skip_connect):
        ...
        for t in range(1,T):
            #initialize at sym =0 (blank)
            alpha[t][0] = logits[t, extended_symbols[0]]*alpha[t-1,0]
            for sym in range(1,S):
                # find prob for previous path
    ...
    def get_backward_probs(self, logits, extended_symbols, skip_connect):
        ...
        for t in range(T - 2, -1, -1):
            ...
            for s in range(S - 2, -1, -1):
                ...
                # find prob of next path
                if s < S - 2 and skip_connect[s + 2]==1:
                    beta[t, s] = beta[t + 1, s + 2] +beta[t + 1, s + 1] +beta[t + 1, s] 
                else: 
                    beta[t, s] = beta[t + 1, s + 1] +beta[t + 1, s] 
                # current step
                beta[t,s] *= logits[t,extended_symbols[s]]
        beta /= logits[:,extended_symbols]
        return beta
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            truncated_target = target[batch_itr, :target_lengths[batch_itr]]
            truncated_logits = logits[:self.input_lengths[batch_itr], batch_itr, :]

            ext_symbols, skip_connect = self.ctc.extend_target_with_blank(truncated_target)
            self.extended_symbols.append(ext_symbols)
            alpha = self.ctc.get_forward_probs(truncated_logits, ext_symbols, skip_connect)
            beta = self.ctc.get_backward_probs(truncated_logits, ext_symbols, skip_connect)
            gamma = self.ctc.get_posterior_probs(alpha, beta)

            self.gammas = gamma  # for backward process

            total_loss[batch_itr] = -np.sum(gamma * np.log(truncated_logits[:,ext_symbols]))
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            truncated_logits = self.logits[:self.input_lengths[batch_itr], batch_itr, :]

            ext_symbols, skip_connect = self.ctc.extend_target_with_blank(truncated_target)
            alpha = self.ctc.get_forward_probs(truncated_logits, ext_symbols, skip_connect)
            beta = self.ctc.get_backward_probs(truncated_logits, ext_symbols, skip_connect)
            gamma = self.ctc.get_posterior_probs(alpha, beta)
            gamma_sum = np.zeros_like(truncated_logits)
            for i in range(C):
                gamma_sum[:,i]=np.sum(gamma[:,ext_symbols == i],axis=1)
            dY[:self.input_lengths[batch_itr],batch_itr,:] = -gamma_sum/truncated_logits
        return dY
