...
class Autograd:
    ...
    def add_operation(self,
        ...
        for i in inputs:
            self.gradient_buffer.add_spot(i)
    ...
    def backward(self, divergence):
        ...
        Returns:
            No return required
        """
        grad_to_update = divergence
        for op in reversed(self.operation_list):
            grad = op.backward_operation(grad_to_update, *op.inputs)
            if not isinstance(grad, tuple):
                grad = (grad,)
            for i, (op_i, grad_i) in enumerate(zip(op.inputs, grad)):
                if op.gradients_to_update[i] is not None:
                    # Ensure grad_update and grad[i] are compatible
                    if grad_i.shape != op.gradients_to_update[i].shape:
                        grad_i = grad_i.reshape(op.gradients_to_update[i].shape)  # Reshape if necessary
                    op.gradients_to_update[i] += grad_i
                else:
                    self.gradient_buffer.update_param(op_i, grad_i)

            grad_to_update = self.gradient_buffer.get_param(op.inputs[0])
